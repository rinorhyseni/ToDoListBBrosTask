...
import { IAddTasksComponentProps } from '../types';

const AddTasksComponent: React.FC<IAddTasksComponentProps> = ({
  labelText,
  addHandler,
  removeHandler,
  completedHandler,
  ...
  const handleOnClick = () => {
    if (!isError.isShow) {
      dispatch(addHandler(title, description, status, user));
      setTitle('');
    }
  };
      ...
      <Droppable droppableId={droppableId}>
        {(provided) => (
          ...
          >
            {selectorState.map(
              (
                { id, title, isFinished, description, status, user },
                index: number
              ) => (
